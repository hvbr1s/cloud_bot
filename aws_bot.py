...
# Initialize Cohere
co = cohere.Client(env_vars["COHERE_API_KEY"])
...
def load_sysprompt(locale):
    filename = f'/home/ubuntu/sam_bot/system_prompt_{locale}.txt'
    try:
...
async def retrieve(user_input, locale, timestamp):
    # Define context box
    contexts = []
...
async def rag(primer, timestamp, contexts, user_id, locale, user_input, platform, joint_query):
    ...
    llm = 'gpt-4-turbo-preview' if platform in ["slack", "discord"] else 'gpt-4-turbo-preview'
...
# Local start command: uvicorn app_frame:app --reload --port 8800
########VM Service Commands#####
